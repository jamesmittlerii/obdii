...
@MainActor func drawGaugeImage(for pid: OBDPID, measurement: MeasurementResult?, size: CGSize)
  -> UIImage
{
  ...
  let metricRanges: [ValueRange] = [pid.typicalRange, pid.warningRange, pid.dangerRange].compactMap
  { $0 }
  ...
  return renderer.image { _ in
    ...
    let progressPath = UIBezierPath(
      arcCenter: center, radius: radius, startAngle: startAngle, endAngle: progressEndAngle,
      clockwise: true)
...
// SwiftUI-specific color mapping for CodeSeverity without colliding with UIKit helper
