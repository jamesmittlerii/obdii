...
final class CarPlayGaugeDetailController: CarPlayBaseTemplateController<GaugeDetailViewModel> {
  ...
  private func buildItems(for pid: OBDPID, stats: OBDConnectionManager.PIDStats?)
    -> [CPInformationItem]
  {
    ...
    if let s = stats {
      ...
      let maxStr = pid.formatted(
        measurement: MeasurementResult(value: s.max, unit: s.latest.unit), includeUnits: true)
