...
// store a value range to show the limits on our gauges
...
// units are a bit of a pain. Mainly because we keep the gauges limits in metric only
...
// our structure for storing all the PID descriptive stuff
struct OBDPID: Identifiable, Hashable, Codable {

  // two types of pids - gauges and non gauges
  // fuel status, mil status, dtcs are non-gauges
...
extension OBDPID {
  ...
  func formatted(measurement: MeasurementResult, includeUnits: Bool = true) -> String {
    ...
    let v =
      formatter.string(from: measurement.value as NSNumber)
...
// our library of pid definitions
struct OBDPIDLibrary {

  // we load it from a JSON file
